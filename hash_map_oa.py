...
class HashMap:
    ...
    def _probe_index(self, key: str) -> int:
        """
        Computes the index for a key
        """
    ...
    def put(self, key: str, value: object) -> None:
        ...
        resizes the table if load is >= 0.5
        """
        # resize if load is >= 0.5
        if self.table_load() >= 0.5:
            self.resize_table(self._capacity * 2)

        index = self._probe_index(key)
        j = 0
        first_tombstone = None

        # start probe
        while j < self._capacity:
            probe = (index + j * j) % self._capacity
            entry = self._buckets[probe]

            # if empty slot is found
            if entry is None:
                # prioritize tombstone if one was found
            ...
            # save first found tombstone
            if entry.is_tombstone:
                if first_tombstone is None:
                    first_tombstone = probe

            # handle the key already having a value
            elif entry.key == key:
                entry.value = value
                return
    ...
    def resize_table(self, new_capacity: int) -> None:
        """
        Resizes capacity to given new_capacity and rehashes all non-tombstone hash table links
        does nothing if new capacity is less than current elements
        """
    ...
    def __iter__(self):
        """
        Enables iteration over hash map
        Initializes _index variable
        """
        self._index = 0
        return self
    ...
    def __next__(self):
        """
        Returns the next active HashEntry
